...
<fieldset class="wizard-fieldset">
	...
	if ( 'checked' === $wpep_donation_goal_switch && ! empty( trim($wpep_donation_goal_amount) ) ) {
		require WPEP_ROOT_PATH . 'views/frontend/donation_goals.php';
	}
	...
	<div class="btnGroup ifSingle">
		...
		<div style="display:flex">
			...
			// echo 'test ' . $wpep_enable_signup_fees;

			// echo '<pre>$fees_data$fees_data';
			// print_r( $fees_data );
			// echo '</pre>';

			//wp_die('wait here!');		

			/*if ( empty( $fees_data[0]['check'] ) || in_array( 'no', $fees_data[0]['check'] ) && $wpep_enable_signup_fees == 'yes' ) { 
				...
				//echo 'total amount is ' . $wpep_signup_fees_value;
			...
			if ( ( ! empty( $fees_data[0]['check'] ) && in_array( 'yes', $fees_data[0]['check'] ) ) || $wpep_enable_signup_fees == 'yes' ) :				
				?>
				<div class="wpep-payment-details-wrapper">
					<a href="#" class="wpep-open-details" data-id="<?php echo $wpep_current_form_id; ?>"><?php echo esc_html__('Payment details', 'wp_easy_pay'); ?></a>
					<div class="wpep-payment-details" id="wpep-payment-details-<?php echo $wpep_current_form_id; ?>">
					<ul>
						...
						<?php if ( $wpep_enable_signup_fees == 'yes' && ( $payment_type == 'subscription' || $payment_type == 'donation_recurring' ) ) :
							$wpep_signup_fees_value = get_post_meta( $wpep_current_form_id, 'wpep_signup_fees_amount', true ); ?>
						<li class="wpep-fee-onetime">
							<span class="fee_name"><?php echo esc_html__('Signup Fee', 'wp_easy_pay'); ?></span>
							<span class="fee_value"><?php echo esc_attr(number_format($wpep_signup_fees_value, 2)) . ' ' . esc_attr($currency); ?></span>
						...
						//echo 'test total amount ' . $total_amount; 
					...
					<b id="dosign" style="display: none;">$</b><small id="amount_display_<?php echo $wpep_current_form_id; ?>"
																	...
																	<?php
																		if ( !empty($fees_data[0]['check']) && in_array('yes', $fees_data[0]['check']) && isset($total_amount) ) :
																			echo number_format($total_amount, 2) . ' ' . $square_currency;
																		elseif ($wpep_enable_signup_fees == 'yes'):
																			echo number_format($total_amount, 2) . ' ' . $square_currency;
